...
export const getProfilSaya = createAsyncThunk(
  ...
  async ({token}) => {
    const {data} = await http().post('sekolah?api=getDataSiswa', {token});
    return data;
  },
);
...
export const updateProfilSayaAction = createAsyncThunk(
  ...
  async inputData => {
    const {data} = await http().post('sekolah?api=updateDataSiswa', inputData);
    return data;
  },
);
...
export const {clearProfilSaya, clearUpdateProfilSaya} = profilSayaSlicer.actions;
