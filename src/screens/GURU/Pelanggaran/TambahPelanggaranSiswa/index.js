...
export default function Index({navigation}) {
  ...
  const [fotoPelanggaran, setFotoPelanggaran] = useState('');
  ...
  const buatPelanggaranSiswa = useCallback(() => {
    if (authGuru?.status === 'berhasil') {
      const {user_id, website_id} = authGuru?.data;
      const {tahunajaran_id} =
        tahunAjaran?.result?.find(
          item => item?.tahunajaran === tahunAjaranSelected,
        ) || {};
      ...
      const {sdm_id = ''} =
        dataGuru?.result?.sdms?.find(item => item?.nama === ditanganiOleh) ||
        {};
  ...
  useEffect(() => {
    if (tambahPelanggaranSiswa?.status?.length > 0) {
      Alert.alert(
        tambahPelanggaranSiswa?.status === 'berhasil' ? 'Sukses' : 'Gagal',
        tambahPelanggaranSiswa?.pesan || '',
        [
          {
            text:
              tambahPelanggaranSiswa?.status === 'berhasil' ? 'OK' : 'Tutup',
            onPress: () => {
              if (tambahPelanggaranSiswa?.status === 'berhasil') {
                navigation.goBack();
                dispatch(clearBuatPelanggaranSiswa());
              } else {
                dispatch(clearBuatPelanggaranSiswa());
              }
            },
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        <View style={styles.form}>
          ...
          <UploadFoto authGuru={authGuru} onResultImage={setFotoPelanggaran} />
